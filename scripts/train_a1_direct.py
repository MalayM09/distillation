"""
scripts/train_a1_direct.py — Ablation A1 trainer (MedSAM -> EdgeUNet direct)

The Capacity Gap empirical anchor (row A1 of docs/02_ABLATION_MATRIX.md).
Distills MedSAM (91M, ViT) directly into EdgeUNet (<3M, CNN) under the same
Hop-1 dense InfoNCE objective used by the A3 cascade's first hop — but with
no intermediate assistant. The expected outcome, per docs/01:

    A1 mean Dice  ~=  B1 mean Dice    (within noise)

i.e., direct 91M -> 3M distillation acts only as a variance regularizer at
this capacity ratio (R ~= 30) and produces no statistically significant
accuracy lift. A1 must reproduce this null result for the central
Capacity-Gap claim of the paper to hold.

This script is structurally a mirror of scripts/train_hop1.py, swapped to:
    - Student   : EdgeUNet (<3M, INT8-deployable)            instead of HA-Net
    - Loss      : Hop1ContrastiveLoss(student_channels=192)  instead of 512
    - Checkpoint: edgeunet_a1_best.pt
...
Synchronized hflip
------------------
hflip is applied identically to image, 256x256 mask, and the W axis (dim=-1)
of the cached MedSAM feature tensor so the student-to-MedSAM spatial
(i, j) correspondence used by the InfoNCE positive-pair construction is
preserved across augmentation.

Resolution-faithful evaluation
------------------------------
Validation Dice / IoU at native ultrasound resolution per docs/03:
EdgeUNet logits are bilinearly upsampled to (H_native, W_native) and
re-thresholded against the original-size GT mask.

Usage
-----
    python scripts/train_a1_direct.py \\
        --busi_root /kaggle/input/busi/Dataset_BUSI_with_GT \\
        --medsam_cache_root /kaggle/input/caisc-v1/medsam_cache \\
        --output_dir /kaggle/working/outputs_a1_direct \\
...
log = logging.getLogger("train_a1_direct")
...
def discover_cases(busi_root: Path, cache_root: Path) -> List[Case]:
    """Pair each BUSI image with its MedSAM cache file. Drop unmatched cases."""
    cases: List[Case] = []
    n_missing = 0
...
class DistillationDataset(Dataset):
    """Training set: (image_256, mask_256, medsam_features).

    hflip-only augmentation, applied synchronously to the image, the 256x256
    mask, AND the W axis (dim=-1) of the cached MedSAM feature tensor — the
    same protocol as scripts/train_hop1.py. Rotation is forbidden because it
    breaks the (i, j) student-anchor / MedSAM-positive correspondence on
    which the InfoNCE positive-pair construction depends.
    """
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Synchronized hflip across all three tensors.
...
class ValDataset(Dataset):
    """Validation: returns (image_256, gt_native, native_hw). No MedSAM features."""
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, Tuple[int, int]]:
        ...
        merged_native = or_merge_masks(case.mask_paths)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="A1 ablation trainer (MedSAM -> EdgeUNet direct).")
...
def main() -> None:
    ...
    # ----- Student (EdgeUNet, fully trainable; <3M budget enforced) -----
    student = EdgeUNet(in_channels=3, num_classes=1).to(device)
    n_student = sum(p.numel() for p in student.parameters())
    assert n_student < 3_000_000, (
        f"EdgeUNet violates the <3M edge-deployment budget in A1: "
        f"got {n_student:,d} parameters."
    )
    log.info("student EdgeUNet params=%s (~%.2fM, <3M ok)",
             f"{n_student:,d}", n_student / 1e6)

    # ----- Loss + projector -----
    # CRITICAL: student_channels=192 because EdgeUNet's bottleneck is 192 ch
    # (vs HA-Net's 512). The InfoNCE projector here is 1x1 conv 192 -> 256
    # plus the same bilinear 8x8 -> 64x64 upsample to MedSAM's grid.
    loss_fn = Hop1ContrastiveLoss(
        student_channels=EdgeUNet.BOTTLENECK_CHANNELS,  # = 192
    ...
    history: List[Dict] = []
    best_dice = -1.0
    best_path = args.output_dir / "edgeunet_a1_best.pt"
    last_path = args.output_dir / "edgeunet_a1_last.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # ----- Checkpoint (EdgeUNet + projector + optimizer for resumability) -----
        ckpt = {
            "epoch": epoch,
            "ablation": "A1_direct_MedSAM_to_EdgeUNet",
